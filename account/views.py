...
INFO_EMAIL = 'general_information@mywebserver.com'
...
class SignupView(PasswordMixin, FormView):
    ...
    def create_user(self, form, commit=True, model=None, **kwargs):
        User = model
        print("this model 1: ", User)
        if User is None:
            User = get_user_model()
            print("this model 2: ", User)
        user = User(**kwargs)
        print("this model 3: ", user, "  oooooo")
...
class SignUpAPI(APIView):
    ...
    def create_account_api(self, user, phone):
        new_account = Account(user=user, main_phone=phone)
        new_account.save()
        return new_account

    def create_client_api(self, account):
        new_client = ClientMember(base_account=account)
        new_client.save()
        return new_client
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            user.save()
            account = self.create_account_api(
                user,
                params['phone']
                )
...
class PasswordResetTokenAPI(APIView):

    def post(self, request, *args, **kwargs):
        params = request.data
        try:
            is_valid_email = EmailAddress.objects.get(email=params['email'])
            code = is_valid_email.create_password_reset_code()
            # from django.core.mail import send_mail
            send_mail(
                code,
                'If you have recieved this code in error, please reply ' + INFO_EMAIL,
                GEN_EMAIL,
                [params['email']],
                fail_silently=False,
            )
...
class PasswordResetAuthAPI(APIView):

    def post(self, request, *args, **kwargs):
        params = request.data
        # print(type(params['pin']))
        try:
            valid_email = EmailAddress.objects.get(email=params['email'])
            # print(settings.RESET_PIN_EXPIRY, " llll ", type(valid_email.reset_password_code))
            if valid_email.reset_password_code == int(params['pin']):
                valid_email.create_password_reset_code()
                account = Account.objects.get(user=valid_email.user)
                return Response({ 
                    'response': '200',
                    'acct_pk': account.pk,
                    'email': params['email']
